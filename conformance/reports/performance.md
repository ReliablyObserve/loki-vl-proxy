...
Runs: pr611-latency-gap-json-filter-pushdown (2026-09-23), pr611-latency-gap-metric-latency-gap (2026-09-23), pr-loki-profile-control (2026-09-24), pr-loki-profile-otel-metadata (2026-09-24).

## Slower than Loki on first load

| shape | range | proxy cold | Loki cold | proxy warm | Loki warm | registry items |
|---|---|---|---|---|---|---|
| instant json volume | instant | 0.35s | 0.13s | 0.33s | 0.14s | `explore-logs-volume-json` |
| logfmt count by level, filter, drop | 24h | 1.44s | 0.69s | 1.39s | 0.00s | `parser-logfmt` |
| logs plain | 24h | 0.60s | 0.41s | 0.15s | 0.41s | `loki_api_v1_query_range` |
...
25 shape×range slower than Loki beyond noise (25% and 50 ms).
...
| `range_function-rate` | 3 × 3 ranges | 4 same, 1 slower | worst 0.37s / 0.48s (24h) | 45.39s / 0.31s | differs |
...
| `semantics/json-filter-pushdown-without-error-drop` | 3 × 5 ranges | 2 faster, 6 same, 1 slower | worst 1.43s / 0.52s (24h) | 1.07s / 0.01s | differs; 3 slower than Loki cold |
...
| `service-name-derivation` | 2 × 2 ranges | 1 faster, 2 same, 1 slower | worst 0.36s / 0.46s (24h) | 121.96s / 0.44s | differs |
| `severity-detected-level-derivation` | 12 × 5 ranges | 7 faster, 2 fixed, 22 same, 2 slower | worst 4.05s / 3.93s (24h) | 0.94s / 0.47s | differs; 11 slower than Loki cold |
