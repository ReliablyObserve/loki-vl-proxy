...
func (p *Proxy) vlGetInner(ctx context.Context, path string, params url.Values) (*http.Response, error) {
	...
	if err != nil {
		p.recordUpstreamObservation(ctx, "vl", http.MethodGet, path, u.Hostname(), serverPort, upstreamErrorStatus(ctx, err), 0, err)
		return nil, err
	}
	start := time.Now()
...
func (p *Proxy) vlPostHTTP(ctx context.Context, path string, params url.Values) (*http.Response, error) {
	...
	release, err := p.admitBackendRequest(ctx, path, params)
	if err != nil {
		p.recordUpstreamObservation(ctx, "vl", http.MethodPost, path, u.Hostname(), serverPort, upstreamErrorStatus(ctx, err), 0, err)
		return nil, err
	}
	start := time.Now()
	resp, err := p.doBackendRequest(req, p.client)
