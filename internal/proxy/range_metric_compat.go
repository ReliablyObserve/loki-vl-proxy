...
func (p *Proxy) proxyStdvarCompatRange(w http.ResponseWriter, r *http.Request, spec statsCompatSpec) bool {
	params := url.Values{}
	params.Set("query", buildStatsCompatQuery(spec, "stddev"))
	...
	body, err = mutateStatsValues(body, func(v float64) float64 { return v * v })
	if err != nil {
...
func (p *Proxy) proxyStdvarCompatInstant(w http.ResponseWriter, r *http.Request, spec statsCompatSpec) bool {
	params := url.Values{}
	params.Set("query", buildStatsCompatQuery(spec, "stddev"))
	...
	body = p.translateStatsResponseLabels(body, r.FormValue("query"))
	w.Header().Set("Content-Type", "application/json")
	_, _ = w.Write(wrapAsLokiResponse(body, "vector"))
	return true
}

func (p *Proxy) proxyBytesRateCompatRange(w http.ResponseWriter, r *http.Request, spec statsCompatSpec, window time.Duration) bool {
	params := url.Values{}
	params.Set("query", buildStatsCompatQuery(spec, "sum_len"))
	if s := r.FormValue("start"); s != "" {
		params.Set("start", formatVLTimestamp(s))
	}
	if e := r.FormValue("end"); e != "" {
	...
	if windowSeconds <= 0 {
		p.writeError(w, http.StatusBadRequest, "invalid range duration for bytes_rate")
		return true
	}
	body, err = mutateStatsValues(body, func(v float64) float64 { return v / windowSeconds })
	if err != nil {
		p.writeError(w, http.StatusBadGateway, err.Error())
		return true
	}
...
func (p *Proxy) proxyBytesRateCompatInstant(w http.ResponseWriter, r *http.Request, spec statsCompatSpec, window time.Duration) bool {
	params := url.Values{}
	params.Set("query", buildStatsCompatQuery(spec, "sum_len"))
	if t := r.FormValue("time"); t != "" {
		params.Set("time", formatVLTimestamp(t))
...
func buildStatsCompatQuery(spec statsCompatSpec, fn string) string {
	var builder strings.Builder
	builder.WriteString(spec.BaseQuery)
	builder.WriteString(" | stats ")
	if len(spec.GroupBy) > 0 {
		builder.WriteString("by (")
		builder.WriteString(strings.Join(spec.GroupBy, ", "))
		builder.WriteString(") ")
	}
	builder.WriteString(fn)
	if spec.Field != "" {
		builder.WriteByte('(')
		builder.WriteString(spec.Field)
		builder.WriteByte(')')
	}
	return builder.String()
}

func mutateStatsValues(body []byte, transform func(float64) float64) ([]byte, error) {
	...
	if !ok {
		return nil, fmt.Errorf("missing stats data payload")
	}
	result, ok := data["result"].([]interface{})
	if !ok {
		// No series is valid; return payload unchanged.
		return body, nil
	}

	for _, seriesRaw := range result {
		series, ok := seriesRaw.(map[string]interface{})
		if !ok {
			continue
		}
		if valueRaw, ok := series["value"].([]interface{}); ok && len(valueRaw) >= 2 {
			value := parseValueToFloat(valueRaw[1])
			valueRaw[1] = strconv.FormatFloat(transform(value), 'f', -1, 64)
			series["value"] = valueRaw
		}
		if valuesRaw, ok := series["values"].([]interface{}); ok {
			for i := range valuesRaw {
				pair, ok := valuesRaw[i].([]interface{})
				if !ok || len(pair) < 2 {
					continue
				}
				value := parseValueToFloat(pair[1])
				pair[1] = strconv.FormatFloat(transform(value), 'f', -1, 64)
				valuesRaw[i] = pair
			}
			series["values"] = valuesRaw
		}
	}

	out, err := json.Marshal(payload)
	if err != nil {
		return nil, fmt.Errorf("encode stats payload: %w", err)
	}
	return out, nil
}
