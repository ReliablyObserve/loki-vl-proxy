...
func (p *Proxy) handlePatterns(w http.ResponseWriter, r *http.Request) {
	...
	if cacheWriteKey == "" {
		// The level of a pattern is derived, so the key carries the
		// derivation's version and setting: cached patterns outlive a
		// deployment by design (patternsCacheRetention).
		cacheWriteKey = "patterns:" + orgID + ":" + r.URL.Query().Encode() + ":" + p.detectedLevelCacheKey()
		if authFP != "" {
	...
	fetchPatterns := func(endpoint string) ([]patternResultEntry, bool) {
		fetchFromParams := func(queryParams url.Values) ([]patternResultEntry, bool) {
			...
			extracted, stats := extractLogPatternsStreamWithStats(resp.Body, stepParam, patternLimit, p.newLogRowLevels())
