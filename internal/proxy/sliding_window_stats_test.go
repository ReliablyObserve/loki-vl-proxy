...
func TestSlidingWindowStats_RowLimitNamesFlag(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	p.handleQueryRange(rec, httptest.NewRequest(http.MethodGet, "/loki/api/v1/query_range?"+params.Encode(), nil))
	if rec.Code < 400 || !strings.Contains(rec.Body.String(), "manual range metric row limit exceeded (20 stats rows in one bucket grid)") || !strings.Contains(rec.Body.String(), "-manual-range-metric-row-limit") {
...
func TestSlidingWindowStats_TumblingTopKUsesOneGrid(t *testing.T) {
	...
	step := rng / 1000
	start := end.Add(-rng)
	emu := &vlStatsRangeEmulator{t: t, rows: syntheticRows(start.Add(-2*step), end.Add(step), start, step, step)}
	srv := httptest.NewServer(emu)
	defer srv.Close()
	p, err := New(Config{BackendURL: srv.URL, Cache: cache.New(0, 0), LogLevel: "error"})
	if err != nil {
	...
	p.storeBackendVersion("v1.50.0", "v1.50.0")
	params := url.Values{
		"query": {`topk(2, sum by (pod) (count_over_time({app="api"}[` + strconv.FormatInt(int64(step.Seconds()), 10) + `s])))`},
		"start": {strconv.FormatInt(start.UnixNano(), 10)},
		"end":   {strconv.FormatInt(end.UnixNano(), 10)},
		"step":  {strconv.FormatFloat(step.Seconds(), 'f', -1, 64)},
	}
	...
	got := decodeMatrix(t, rec.Body.Bytes())
	all := expectedWindowMatrix(emu.rows, "count_over_time", true, start, end, step, step)
	want := map[string][][2]string{`{pod="pod-b"}`: all[`{pod="pod-b"}`], `{pod="pod-c"}`: all[`{pod="pod-c"}`]}
	if fmt.Sprint(got) != fmt.Sprint(want) {
		t.Fatalf("tumbling topk differs:\n got %v\nwant %v", head(got[`{pod="pod-c"}`]), head(want[`{pod="pod-c"}`]))
	}
	if len(emu.calls) != 1 || emu.rawCalls != 0 {
		t.Fatalf("tumbling topk made %d stats calls and %d raw fetches, want 1 and 0", len(emu.calls), emu.rawCalls)
	}
}
