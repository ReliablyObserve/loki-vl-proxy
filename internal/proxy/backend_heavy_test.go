...
func TestIsHeavyBackendRequest(t *testing.T) {
	...
	cases := []struct {
		...
		{"bounded instant stats", "/select/logsql/stats_query", url.Values{"query": {`* | stats count()`}, "start": {ns(5 * time.Minute)}, "end": {end}, "time": {end}}, false},
		{"unbounded instant stats", "/select/logsql/stats_query", url.Values{"query": {`* | stats count()`}, "time": {end}}, true},
...
func TestHeavyQueryAdmission_QueueFullDoesNotOpenCircuitBreaker(t *testing.T) {
	...
	if shouldRecordBreakerFailure(context.Background(), err) {
		t.Fatal("a rejected admission was counted as a backend failure")
	}
	if shouldRecordBreakerFailure(context.Background(), fmt.Errorf("wrapped: %w", err)) {
		t.Fatal("a wrapped admission rejection was counted as a backend failure")
	}
	...
	p, err2 := New(Config{
		...
		LogLevel:                         "error",
		BackendMaxConcurrentHeavyQueries: 1,
		BackendHeavyQueryQueueWait:       10 * time.Millisecond,
		CBFailThreshold:                  2,
	})
	if err2 != nil {
		t.Fatal(err2)
	...
	params := url.Values{"query": {"* | stats count()"}, "start": {strconv.FormatInt(time.Now().Add(-24*time.Hour).UnixNano(), 10)}, "end": {strconv.FormatInt(time.Now().UnixNano(), 10)}, "step": {"60s"}}
	holder := make(chan struct{})
	go func() {
		defer close(holder)
		resp, err := p.vlPost(context.Background(), "/select/logsql/stats_query_range", params)
		if err == nil {
			_ = resp.Body.Close()
		}
	}()
	...
	for i := 0; i < 5; i++ {
		if _, err := p.vlPost(context.Background(), "/select/logsql/stats_query_range", params); !isHeavyQueryQueueFull(err) {
			t.Fatalf("call %d: expected a queue-full rejection, got %v", i, err)
		}
	}
	if !p.breaker.Allow() {
		t.Fatal("circuit breaker opened after admission rejections")
	}
}
...
func TestHeavyQueryAdmission_GrafanaStatsFailureKeeps429(t *testing.T) {
	...
	t.Cleanup(func() { _ = p.Shutdown(context.Background()) })
	rec := httptest.NewRecorder()
	p.writeGrafanaStatsFailure(rec, &heavyQueryQueueFullError{maxConcurrent: 2, queueWait: 20 * time.Second})
	if rec.Code != http.StatusTooManyRequests {
		t.Fatalf("status %d, want 429; body=%s", rec.Code, rec.Body.String())
	}
	if body := rec.Body.String(); !strings.Contains(body, "too many outstanding requests") || strings.Contains(body, "partial") {
		t.Fatalf("Grafana stats failure body = %s", body)
	}
}
...
func TestHeavyQueryAdmission_QueueWaitIsPerRequest(t *testing.T) {
	p, err := New(Config{
		...
		LogLevel:                         "error",
		BackendMaxConcurrentHeavyQueries: 1,
		BackendHeavyQueryQueueWait:       150 * time.Millisecond,
	})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = p.Shutdown(context.Background()) })
	hold, _ := p.heavyQueryLimiter.acquire(context.Background(), "")
	defer hold()

	ctx := context.WithValue(context.Background(), requestTelemetryKey, newRequestTelemetry())
	heavy := url.Values{"query": {"* | stats count()"}, "start": {strconv.FormatInt(time.Now().Add(-24*time.Hour).UnixNano(), 10)}, "end": {strconv.FormatInt(time.Now().UnixNano(), 10)}, "step": {"60s"}}
	started := time.Now()
	for i := 0; i < 3; i++ {
		if _, err := p.admitBackendRequest(ctx, "/select/logsql/stats_query_range", heavy); !isHeavyQueryQueueFull(err) {
			t.Fatalf("call %d: %v", i, err)
		}
	}
	if waited := time.Since(started); waited > 400*time.Millisecond {
		t.Fatalf("three heavy calls of one request waited %s, want about one queue wait", waited)
	}
}
