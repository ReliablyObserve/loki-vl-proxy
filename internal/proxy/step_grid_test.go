...
func TestAlignRangeRequestToStepGrid(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			alignRangeRequestToStepGrid(r, tc.query, time.Now())
...
func TestAlignRangeRequestToStepGrid_NoStepIsANoOp(t *testing.T) {
	...
	alignRangeRequestToStepGrid(r, `sum(rate({app="x"}[5m]))`, time.Now())
	if got := r.FormValue("start"); got != "1700000000000000000" {
		t.Errorf("start = %q, want it untouched without a step", got)
	}
}

// Loki aligns the bounds it derives for a request without start or end
// (end=now, start=end-since) the same way, so the proxy writes the aligned
// derived bounds into the request.
func TestAlignRangeRequestToStepGrid_DerivesLokiDefaultBounds(t *testing.T) {
	now := time.Unix(1700000000, 0) // 12408759*137 + 17
	for _, tc := range []struct {
		name, rawQuery string
		wantStart      int64
	}{
		{"no bounds: end=now, start=now-1h", "step=137", 1699996284}, // 1699996400 = 12408732*137 + 116
		{"since only", "step=137&since=10m", 1699999298},             // 1699999400 = 12408754*137 + 102
	} {
		t.Run(tc.name, func(t *testing.T) {
			r := httptest.NewRequest(http.MethodGet, "/loki/api/v1/query_range?"+tc.rawQuery, nil)
			alignRangeRequestToStepGrid(r, `sum(rate({app="x"}[5m]))`, now)
			gotStart, _ := parseLokiTimeToUnixNano(r.FormValue("start"))
			gotEnd, _ := parseLokiTimeToUnixNano(r.FormValue("end"))
			if gotStart != tc.wantStart*int64(time.Second) {
				t.Errorf("start = %d, want %d", gotStart/int64(time.Second), tc.wantStart)
			}
			if gotEnd != (1700000000-17)*int64(time.Second) {
				t.Errorf("end = %d, want %d", gotEnd/int64(time.Second), 1700000000-17)
			}
			if q := r.URL.Query().Get("end"); q != r.FormValue("end") {
				t.Errorf("URL end = %q, form end = %q", q, r.FormValue("end"))
			}
		})
	}
}
...
// The alignment reads the step with the same parser as the range evaluators
// and the 11,000-point check, including the raw nanosecond integer Grafana
// sometimes sends. That spelling is a proxy extension (Loki reads a bare
// integer as seconds); what matters is that every consumer of the step in one
// request agrees on it, so a panel is never aligned on one grid and evaluated
// on another.
func TestAlignRangeRequestAcceptsNanosecondStep(t *testing.T) {
	for _, step := range []string{"137s", "137", "137000000000"} {
		...
		alignRangeRequestToStepGrid(r, `sum(rate({app="a"}[5m]))`, time.Now())
...
// The 11,000-point check reads the step exactly as the alignment and the
// evaluators do, so a nanosecond-integer step is neither aligned on one grid
// and limited on another, nor exempt from the limit. (A bare integer small
// enough to be seconds is seconds; the nanosecond reading applies to the
// integers Grafana sends, which overflow as seconds.)
func TestResolutionLimitReadsStepLikeTheEvaluator(t *testing.T) {
	now := time.Unix(1700000000, 0)
	const rangeDur = 130 * 24 * time.Hour // 11,232 steps of 1000s
	for _, tc := range []struct {
		step string
		want bool
	}{
		{"1000s", true},
		{"1000000000000", true}, // the same step as a nanosecond integer
		{"2000s", false},
		{"2000000000000", false},
	} {
		r := httptest.NewRequest(http.MethodGet, "/loki/api/v1/query_range?start="+strconv.FormatInt(now.Add(-rangeDur).UnixNano(), 10)+"&end="+strconv.FormatInt(now.UnixNano(), 10)+"&step="+tc.step, nil)
		if got := exceedsLokiRangeResolution(r, now); got != tc.want {
			t.Errorf("step=%s: exceeds = %v, want %v", tc.step, got, tc.want)
		}
	}
}
