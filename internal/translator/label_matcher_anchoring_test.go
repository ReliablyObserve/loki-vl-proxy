...
func TestLabelMatchersAreAnchoredLineFiltersAreNot(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:   `{namespace=~"(?i)prod"}`,
			want: `namespace:~"^(?s:(?i)prod)$"`,
		},
		{
			name: "multiline flag cannot widen the anchors",
			in:   `{namespace=~"(?m)prod"}`,
			want: `namespace:~"^(?s:(?m)prod)$"`,
		},
		{
