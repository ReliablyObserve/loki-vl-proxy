...
func TestRealWorld_NegativeLineFilters(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want:  `instance:~"^(?s:kafka-[23])$" name:="kafka" NOT ~"kafka\\.server:type=ReplicaManager"`,
		},
		{
...
func TestRealWorld_ComplexKubernetes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want:  `cluster:~"^(?s:eks-.*)$" namespace:="payments" | unpack_json | filter status:>=500`,
		},
		{
			name:  "container restart correlation",
			logql: `{namespace=~"prod.*",container!=""} |= "Back-off restarting failed container"`,
			want:  `namespace:~"^(?s:prod.*)$" container:!"" ~"Back-off restarting failed container"`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
