...
func (p *OTLPPusher) cacheTierMetrics(now int64) []map[string]interface{} {
	...
	return []map[string]interface{}{
		p.sumMetric(
			...
			"Cache tier lookup attempts by tier. tier=l0 is the hot-key index sidecar (not a lookup tier — counts every cache request that touched it).",
			"",
			p.counterDP("loki_vl_proxy_cache_tier_requests_total", stats.L0.Requests, now, attr("tier", "l0")),
		...
		p.sumMetric(
			...
			"Cache hits by tier. tier=l0 = cache requests where the key was already in the hot-key index (was hot before this request).",
			"",
			p.counterDP("loki_vl_proxy_cache_tier_hits_total", stats.L0.Hits, now, attr("tier", "l0")),
		...
		p.sumMetric(
			...
			"Cache misses by tier. tier=l0 = cache requests for keys not yet in the hot index (first observation of that key).",
			"",
			p.counterDP("loki_vl_proxy_cache_tier_misses_total", stats.L0.Misses, now, attr("tier", "l0")),
		...
		p.gaugeMetric(
			...
			"Current object count by cache tier. tier=l0 = current bounded population of the hot-key index.",
			"",
			p.gaugeDP("loki_vl_proxy_cache_objects", float64(stats.HotEntries), now, attr("tier", "l0")),
