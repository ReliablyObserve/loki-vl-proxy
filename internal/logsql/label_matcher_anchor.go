...
//
// The group is Prometheus' own "(?s:", so the emitted pattern is literally the
// one Loki compiles. On VictoriaLogs the s flag changes nothing — it compiles
// with DotNL already on (measured on v1.52.0, field:~"a.b" matches "a\nb") —
// but the same helper feeds a Go regexp.Compile in the multi-tenant path, where
// "." would otherwise stop at a newline.
//
...
// becomes `^(?s:(?m)foo)$`, where the outer `^`/`$` still mean the whole value.
//
// An empty pattern is anchored like any other: Loki's =~"" matches only the
// empty value, so it becomes `^(?s:)$` rather than an unanchored match-all.
//
