...
Every metric path applies the series limit the same way: raw and ordered-parser evaluators, stats and hits buckets, sliding windows, binary expressions and unions, multi-tenant merges, range and instant. Logs Drilldown's partial result holds the busiest series by total value, where Loki keeps the first series it encounters. One Drilldown route differs on purpose: a single-field breakdown with a `field!=""` filter answers from `/hits` top values (`X-Proxy-Drilldown-Path: hits`, window-sampled from two hours on), which holds fewer series than the limit and no warning.
