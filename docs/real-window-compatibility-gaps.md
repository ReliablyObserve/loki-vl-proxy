...
- Binary expressions joining two vector operands evaluate both on the step-aligned grid, as Loki's query frontend does when `align_queries_with_step` is enabled, whether or not `-align-queries-with-step` is set on the proxy. The compatibility stack enables both; a default Loki evaluates at `start+k*step`, so a binary join with an unaligned start differs from it by less than one step. Operands with different ranges take different execution paths (sliding evaluation versus tumbling buckets), and an unaligned start previously left no common timestamps to join.
...
- With `-align-queries-with-step`, every metric range query (not only binary joins) evaluates on Loki's `k*step` grid: both bounds truncate down to a multiple of the step, as Loki's step-align middleware does. Measured on the stack at `step=137` with an unaligned start, Loki and the proxy return the same 22 timestamps where an unaligned proxy returns 21 starting 26s late.
