...
| `loki_vl_proxy_parser_metric_evaluations_total` | counter | `evaluator`, `reason` | `Low` | metric queries over a `\| json` or `\| logfmt` pipeline by the evaluator that answered them (`vl_stats_buckets`: VictoriaLogs stats over unpacked fields, no raw rows; `raw_rows`: the exact row evaluator, bounded by `-manual-range-metric-row-limit`) and why: `pushdown`; `probe` (a `\| limit 1` probe found a line the two parsers read differently, so the line shapes of that stream are worth a look); `ineligible` (a pipeline the pushdown does not cover: `keep`, a second parser, an `_extracted` or `__error__` label, a filter accepting the empty value without `drop __error__`); `grid` (no bucket grid for the step and range); `instant` |
