...
| `loki_vl_proxy_range_metric_evaluations_total` | counter | `evaluator` | `Low` | parser-dependent range metric queries (`\| json` / `\| logfmt` under a range function) by the evaluator that answered them: `vl_stats_buckets` (VictoriaLogs stats over unpacked fields, no raw rows) or `raw_rows` (the exact row evaluator kept when a parse-risk probe finds a line the two parsers read differently); a rising `raw_rows` rate on a stream is the signal to inspect its line shapes |
