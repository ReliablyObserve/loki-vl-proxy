...
- a `stats_query_range`, `stats_query` or `hits` call spanning at least `-backend-heavy-query-min-range`, or with a bucket grid finer than 11,000 buckets (Loki's resolution limit).
