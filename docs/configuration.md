...
| `-exact-parser-series-identity` | — | `false` | Name the series of a metric over `\| json` or `\| logfmt` with the labels those parsers extracted, as Loki does. Loki's series identity is the stream labels plus every extracted label, so `count_over_time({app="x"} \| logfmt [1m])` returns one series per distinct set of parsed values (450 on a 30-stream minute of the e2e generator) where the proxy returns one per stream (30). Turning this on evaluates every such query from rows (bounded by the manual row budget) instead of pushing it down to VictoriaLogs stats, so it costs far more on wide ranges; leave it off unless a dashboard depends on those labels. Captures of `\| regexp` and `\| pattern` are always part of the identity, because the query names them and VictoriaLogs can group by them |
...
| `-backend-heavy-query-min-range` | — | `6h` | Time range from which `stats_query_range`, `stats_query`, `hits` and unbounded raw `query` calls count as heavy, and metadata listings count as long-range scans. Must be `> 0` |
| `-backend-max-concurrent-metadata-scans` | — | `2` | Maximum concurrent long-range VictoriaLogs metadata listings per replica (`stream_field_names`, `stream_field_values`, `field_names`, `field_values`, `streams` spanning at least `-backend-heavy-query-min-range`), on their own limiter (see [Heavy VictoriaLogs Query Admission](#heavy-victorialogs-query-admission)). `0` disables the limiter |
...
Loki answers `/labels` and `/label/{name}/values` from its index in milliseconds whatever the range. VictoriaLogs has no such index: a `stream_field_names`, `stream_field_values`, `field_values` or `streams` listing scans every partition in the range, and VictoriaLogs admits any number of them (`-search.maxConcurrentRequests`) without accounting for their combined memory. Measured on v1.52.0 with a 7-day retention, one 7-day `stream_field_names` scan holds about 0.8 GiB for 9 s; three at once hold 2 GiB more and take 16 s each; seven at once, sent by seven proxy replicas whose label keep-warm ticks coincided, OOM-killed an 8 GiB VictoriaLogs three times in seven minutes. Concurrency past two buys memory, not throughput.

A metadata listing spanning at least `-backend-heavy-query-min-range`, or without a time range, therefore goes through its own limiter, `-backend-max-concurrent-metadata-scans` (default `2` per replica). It is separate from the heavy-query limiter so a 7-day label browser never waits behind 7-day charts, and shorter listings are never queued at all. A synchronous request (Explore's label browser, `/series`, a 7-day `/labels`) waits up to `-backend-heavy-query-queue-wait` for a slot and then fails with HTTP `429`, `too many outstanding requests: long-range VictoriaLogs metadata scans are limited to -backend-max-concurrent-metadata-scans=N per replica and this query waited -backend-heavy-query-queue-wait=D`. Background inventory work — the startup warm-up, the keep-warm loop and stale-entry refreshes of the 1h/6h/24h/7d presets — never waits: a refresh that finds every slot busy is skipped and retried on its next schedule, the cached answer keeps serving, and a preset window whose warm-up failed backs off (one keep-warm interval, doubling to one hour) instead of retrying on every tick. The keep-warm interval is jittered by up to a quarter so replicas started together do not tick in step. Identical concurrent listings within a replica share one backend call; the peer cache serves warm-up windows another replica already holds. The limit is per replica: size VictoriaLogs for about 1 GiB per slot per week of retention across the fleet, and lower the value to `1` with many replicas or a small VictoriaLogs. Admissions, waits and rejections are exported as `loki_vl_proxy_internal_operation_total{operation="backend_metadata_scan_admission"}`.
