...
- **LogQL label-matcher regexps are anchored to the whole label value.**
  ...
  `labels.NewMatcher` compiles a `MatchRegexp` as `^(?s:<pattern>)$` — while
  ...
  it. Stream-selector matchers and pipeline label filters (`| status =~ "5.."`)
  now emit Prometheus' `^(?s:…)$`; the derived `service_name` matcher, which
  already anchored its pattern, shares the same helper. Line filters (`|~`,
  `!~`) are unchanged: those are substring regexps in Loki, which is what
  VictoriaLogs already does.
