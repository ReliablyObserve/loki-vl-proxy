...
### Performance
- **Hybrid drilldown path for wide ranges (>6h)**: `proxyStatsQueryRangeDrilldownHybrid` replaces the full-range `stats_query_range` scan with a two-tier approach for time ranges above 6h. Tier 1: `field_values` over the full range (~30ms, O(column-index)) discovers all unique values and their total hit counts. Tier 2: `stats_query_range` over an adaptive recent window only (`rangeNs/8`, bounded [1h, 24h]) provides histogram bars for the sparklines. The two results are merged: values present in the histogram keep their per-bucket bars; values seen only in `field_values` get a single stub point at the end of the range scaled to a per-bucket-equivalent count. High-cardinality fields (`trace_id`, `span_id`, `*_id`, `*_uid`) skip Tier 2 entirely and return a pure `field_values` synthesis (~30ms regardless of range). Reports the chosen path in `X-Proxy-Drilldown-Path` response header (`hybrid/fv+<window>-stats`, `field_values_hc`, `field_values_hc_empty`, `field_values_stats_err`, `empty`). Before this change, the proxy returned an empty matrix for all fields on 12h+ ranges (the two-phase fallback ran but was capped to 500 series per bucket, making it effectively useless for wide ranges).
