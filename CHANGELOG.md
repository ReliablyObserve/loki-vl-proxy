...
- fix(metrics): `fetchBareParserMetricSeries` no longer leaks parsed fields (e.g. `status`, `method`) into metric series labels — only stream labels from the original selector are used, matching Loki's bare-parser metric cardinality
