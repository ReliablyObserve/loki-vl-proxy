...
### Performance
- **Drilldown response cache (60s)**: `proxyStatsQueryRangeDrilldown` now caches the translated Loki matrix response for 60 seconds (keyed on orgID + LogQL + time range, with `end` bucketed to 5-min to absorb Grafana's sliding window). The cache is checked before acquiring the concurrency semaphore so re-renders (Grafana ticks, filter interactions, second tab) are served with zero VL calls and consume no semaphore slot. On first load the semaphore still limits concurrent VL calls to 4; any re-render within 60s costs nothing. The existing `query_range` cache (10s TTL) still provides short-term deduplication; this cache adds the longer drilldown-specific window.
- **Eager two-phase for long-range Drilldown queries**: `drilldownShouldEagerTwoPhase` now triggers two-phase when the query spans more than 60 time buckets (e.g., 1h at 60s step, 15min at 15s step). The direct path sends `| sort by (_c desc) | limit 500` to VL which it applies **per bucket** — a 6h/60s query (360 buckets) runs 360 sort passes across all matching logs, one per bucket. Two-phase runs a single-bucket Phase 1 (one global sort, ~40 KB) then a Phase 2 filtered to the top-500 values (`field:in(...)`, no per-bucket sort). Combined with the 60s cache, this eliminates both the initial per-query sort overhead and all re-render VL calls.
- Eager two-phase for high-cardinality Drilldown fields: `proxyStatsQueryRangeDrilldown` now skips the direct-path VL attempt entirely when overflow is predictable before any byte is read — eliminating the read-and-reject cycle (up to 32 MB wasted read + VL scan CPU spike) for two cases: (1) known UUID/snowflake field names (`trace_id`, `span_id`, `*_id`, `*_uuid`, `*_token`, `*_hash`, `*_key`) detected by `isLikelyHighCardinalityField`; (2) any field previously observed to overflow, tracked in `drilldownCardinalityCache` (5-min TTL per `{orgID, base, field}`) so Grafana re-renders (filter/time-range changes) skip the direct attempt without another wasted round-trip. The previous bucket-count overflow signal (`numBuckets × 500 × 200 B > 32 MB`) was removed: for a 6h/60s range (360 buckets) it triggered eager two-phase for every field unconditionally, doubling VL calls and driving a 24h Phase 1 full-range scan for all fields regardless of cardinality.
