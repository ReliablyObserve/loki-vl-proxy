...
- **Heavy VictoriaLogs query admission.** At most
  ...
  (default `20s`, shared by every heavy call of one request) and then fail like
  Loki's full query scheduler queue with HTTP 429 `too many outstanding
  requests`, naming both flags — for Grafana-sourced requests too, where the
  partial-result carve-out covers the series limit only. A rejection never
  counts as a backend failure, so it cannot open the circuit breaker. A released
  slot goes to the waiting tenant holding the fewest slots. Metadata lookups are
