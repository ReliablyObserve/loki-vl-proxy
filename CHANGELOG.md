...
- **Long-range metric queries no longer exhaust VictoriaLogs memory.** On the
  ...
  than 11,000 buckets, and for every `topk`/`bottomk` input, the proxy now
  reads one window-sized seed bucket and two step-sized bucket grids from a
  VictoriaLogs stats pipe (`/select/logsql/query ... | stats by (_time:step
  offset ...)`) and derives every window exactly from
  `W(k+1) = W(k) + A(k) - C(k)`. VictoriaLogs streams at most one row per step
  and series, and the proxy keeps one compact record per row. Backend errors
  on this path are returned instead of retried on a raw fetch.
- **`topk`/`bottomk` over range metrics rank every series.** The inner
  ...
  all series, as Loki ranks per step.
...
- **Heavy VictoriaLogs query admission.** At most
  `-backend-max-concurrent-heavy-queries` (default `2`, `0` disables) heavy
  VictoriaLogs calls run per replica: raw-row metric fetches above 10,000 rows,
  and `stats_query_range`, `stats_query`, `hits` or unbounded `query` calls
  spanning at least `-backend-heavy-query-min-range` (default `6h`) or finer
  than 11,000 buckets. Others wait up to `-backend-heavy-query-queue-wait`
  (default `20s`) and then fail like Loki's full query scheduler queue with
  HTTP 429 `too many outstanding requests`, naming both flags. A released slot
  goes to the waiting tenant holding the fewest slots. Metadata lookups are
  never queued. Helm: `extraArgs.backend-max-concurrent-heavy-queries`,
  `extraArgs.backend-heavy-query-queue-wait`,
  `extraArgs.backend-heavy-query-min-range`. Invalid values are rejected at
  startup.

### Changed

- Queries whose heavy VictoriaLogs calls cannot get an admission slot within
  `-backend-heavy-query-queue-wait` now return HTTP 429 instead of running.
  Size `-backend-max-concurrent-heavy-queries` for the fleet: the limit is per
  replica.
- Sliding-window metrics answered by the window stats path return fractional
  sample timestamps for fractional steps (for example `...86.4`), as Loki does.

## [1.80.1] - 2026-09-15

### Fixed

- **Metric queries now keep the labels Loki returns when grouping by `level` or `service_name`.**
  `sum by (level) (...)` came back grouped by `detected_level`; Loki returns the
  `level` label the query named, and returns `detected_level` only when the query
  groups by it. With the default underscore label style, grouping by
  `service_name` translated to VictoriaLogs `stats by (service.name)` on instant
  queries and on the outer stats pipe of `rate()`, `bytes_over_time()` and
  similar pipelines, so every stream stored with a plain `service_name` field
  (Loki push, JSON lines) was grouped under an empty `service_name` together.
  Each stats pipe now also groups by the underscore field and the two are
  merged into one Loki label, as range `count_over_time` already did.
- **Range metric queries between 10,000 and 11,000 points no longer fail.**
  The `| json` metric evaluator rejected more than 10,000 evaluation points,
  while Loki accepts up to 11,000 per series. Grafana splits a 24-hour logs
  volume into chunks whose `$__auto` step can land in that band, so Explore
  showed "maximum 10000 evaluation points" where Loki draws the chart. The
  evaluator now uses Loki's limit and Loki's error message.
...
- **e2e stack: VictoriaLogs compatibility matrix and latest-release support.**
  ...
  - `compatibility-matrix.json` gains v1.51.0 and v1.52.0, and
    `compat-vl.yaml` runs the full version matrix on `workflow_dispatch` as
    well as on the weekly schedule, as the compatibility docs already promised.
  - The same `wget` probe was used by the root `docker-compose.yml`, the
    e2e-fleet and e2e-pipeline stacks and every `examples/` compose file; the
    examples pin `victoria-logs:latest`, so the public quickstart could not
    start its proxy once `latest` moved to v1.52.0. All of them now probe the
    binary.
  - The e2e log generator additionally waits for the main proxy to be healthy
    (its `/ready` probes VictoriaLogs' `/health`), so the first batch is not
    pushed before VictoriaLogs accepts connections; a dual write is not atomic
    and a lost first batch would be a permanent delta in every window that
    includes stack start.
- **e2e-ui: Logs Drilldown 2.5.x selectors.** The landing-page and service-page
  ...
  on Drilldown 2.5.x `getByRole("combobox", { name })` times out for them
  while `getByPlaceholder` resolves the same input (observed on 2.5.2), so
  every drilldown-core / drilldown-multitenant spec failed against the latest
  plugin while the proxy itself rendered the pages correctly. The
  `@drilldown-cache` suite uses the same helper.
- **e2e-ui: `@regression` and `@comprehensive-ui` suites repaired and wired
  ...
  `-max-stats-query-series` (500), and compared `limit`-capped log responses
  (500 lines against 500 lines, which proves nothing at the generator's
  ~1,000 lines/min). Log parity now compares a two-minute window with a
  5,000-line limit and fails if the window is empty or capped; metric parity
  uses bounded aggregations; volume comparisons use instant `count_over_time`
  aggregates; and the series cap is locked by its own test over a 15-minute
  window under a "Proxy series cap" describe, since it documents a proxy
  deviation rather than Loki parity. The comprehensive suite now asserts a
  rendered graph for its (previously invalid) unwrap query, checks every test
  for Grafana errors, and records timings as annotations instead of asserting
  wall-clock bounds on shared runners.
- **e2e-ui: proxy-vs-Loki metric comparisons account for Loki's fresh-stack
  ...
  readiness, so grouped comparisons would have failed with "Loki returned 0
  series" and raw per-stream counts could never match. The new
  `waitForLokiMetricData` helper (modelled on the Go suite's helper of the same
  name, but stricter: it waits for group completeness against Loki's series
  index and fails instead of warning) gates grouped comparisons until Loki's
  range path reports every level/namespace group its own series index lists,
  tolerating transient request errors while polling; the per-stream series-cap
  test counts streams against Loki's series index (`lokiIndexedStreamCount`),
  which is complete at any age, instead of Loki's `rate()` output.
