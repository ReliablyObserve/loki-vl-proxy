#!/bin/zsh
ROOT=/Users/slawomirskowron/claude_projects/loki-vl-proxy; OUT=$ROOT/scratchpad/codify-pinned/final2
...
for tag in @explore-core @regression @drilldown-core @drilldown-mt @comprehensive-ui; do
  ...
  echo "$(date +%T) final2 $name: ${p:-0 passed} ${f:-0 failed} ${s:-0 skipped} ${fl:-} $d"
done
echo FINAL2_DONE
